...
def define_argparser():
    ...
    parser.add_argument(
        ...
        nargs='*', 
        type=list, 
        help="Features list for xgboost"
    )
    parser.add_argument(
        "--fixed_param_xgb", 
        default={
        ...
        nargs='+', 
        type=dict, 
        help="Fixed parameter dictionary for xgboost"
    )
    parser.add_argument(
        "--param_grid_xgb", 
        default={
        ...
        nargs='+', 
        type=dict, 
        help="Paramer dictionary to apply grid search for xgboost"
    )
    ...
    parser.add_argument(
        ...
        nargs='+', 
        type=dict, 
        help="Dictionary of RandomForest regression parameters"
    )
    parser.add_argument(
        "--linear_param", 
        default={
        ...
        nargs='+', 
        type=dict, 
        help="Dictionary of Linear regression parameters"
    )
    parser.add_argument(
        "--elastic_param", 
        default={
        ...
        nargs='+', 
        type=dict, 
        help="Dictionary of ElasticNet parameters"
    )
...
def main(args):
    ...
    xgboost_results_lunch = grid_search(
    ...
    xgboost_results_dinner = grid_search(
        train_data=train_data,
        parameter_candidate=args.param_grid_xgb, 
        model_name=xgboost,
        basic_param=args.fixed_param_xgb, 
    ...
    xgboost_lunch_results = (
        xgboost_results_lunch
        .groupby(['parameter'])['loss']
        .apply(np.mean)
        .reset_index(name='loss')
        .assign(
            type = 'lunch',
            model = xgboost.__name__,
        )
    )


    xgboost_dinner_results = (
        xgboost_results_dinner
        .groupby(['parameter'])['loss']
        .apply(np.mean)
        .reset_index(name='loss')
        .assign(
    ...
    for i in range(len(args.models_list)):
        ...
        lunch_results = (
            ...
            .assign(
                type = 'lunch',
                model = args.models_list[i].__name__,
            )
        )
